...
module ActiveRDF
  ...
  class SesameAdapter < ActiveRdfAdapter
    ...
    #
    # = :bigowlim
    # Implements BigOWLIM SailRepository. 
    # [*location*] - Path to the data file for BigOWLIM
    # [*ruleset*] - 'rdfs', 'owl-horst' (default), 'owl-max', 'owl2-rl', 'rdfs-optimized', 'owl-horst-optimized', 
    #               'owl-max-optimized' and 'owl2- rl-optimized'. See BigOWLIM User Guide.
    ...
    def load_namespaces
      namespaces = @db.getNamespaces.asList.to_a.map{|v| [v.getPrefix, v.getName]}
      namespaces.map!{|ns| [ns.first.to_sym, ns.last] unless ns.first.nil? || ns.first.blank? }
      namespaces.compact!
      namespaces.map{|ns| 
        begin
          ActiveRDF::Namespace.register(*ns)
        rescue => ex
        end
        }
    end
    ...
    def set_namespace(prefix, uri)
      @db.setNamespace(prefix.to_s, uri)
    end

    def remove_namespace(prefix)
      @db.removeNamespace(prefix.to_s)
    end    
    ...
    def init_bigowlim_store(params)      
