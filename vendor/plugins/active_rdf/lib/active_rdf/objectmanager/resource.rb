...
module RDFS
  ...
  class RDFS::Resource
    ...
    def Resource.uri
      class_uri.uri unless class_uri.nil?
    end
    ...
    # options[:context] 
    # options[:datasets] federated datasets
    def Resource.find_all(options = {}, &blk)
      contexts = options.delete(:context)
      datasets = options.delete(:datasets)
      ActiveRDF::ResourceQuery.new(self, contexts).datasets(*datasets).execute(options,&blk)
    end
    ...
    def Resource.find_by(context = nil)
      query = ActiveRDF::ResourceQuery.new(self, context)
    end
    ...
    def initialize(uri_or_resource, datasets=nil)
      @datasets = datasets    
      @uri = case uri_or_resource
             ...
             when String
               ...
               raise ActiveRDF::ActiveRdfError, "cannot create resource <#{uri_or_resource}>"
             end
    end
    ...
    def new_record?
      new_query.count(:p).where(self,:p,:o).execute == 0
    end
    ...
    def contexts
      new_query.distinct(:c).where(self,nil,nil,:c).execute
    end
    ...
    if $activerdf_internal_reasoning
      ...
      def instance_predicates
        preds = new_query.distinct(:p).where(:p,RDFS::domain,self).execute
        preds |= preds.collect{|p| p.super_predicates}.flatten
        preds |= super_types.collect{|type| type.instance_predicates}.flatten
        preds |= new_query.distinct(:p).where(:p,RDFS::domain,RDFS::Resource).execute  # all resources share RDFS::Resource properties
        preds
      end
      ...
      def super_types
        sups = new_query.distinct(:super_class).where(self,RDFS::subClassOf,:super_class).execute
      ...
      def super_predicates
        sups = new_query.distinct(:super_property).where(self, RDFS::subPropertyOf, :super_property).execute
      ...
      def sub_predicates
        subs = new_query.distinct(:sub_property).where(:sub_property, RDFS::subPropertyOf, self).execute
    ...
    def inspect
      if ActiveRDF::ConnectionPool.adapters.size > 0
        ...
        #    uri
        #  end 
        #  MAURICIO: removed to improve performance
        label = self.uri
      else
        type = self.class
        label = self.uri
    ...
    def to_xml
      ...
      direct_predicates.each do |p|
        objects = new_query.distinct(:o).where(self, p, :o).execute
        objects.each do |obj|
    ...
    #create a query with federated datasets
    def new_query
      query = ActiveRDF::Query.new
      query.datasets(*@datasets) unless (@datasets.nil? || @datasets.empty?)
      query
    end
