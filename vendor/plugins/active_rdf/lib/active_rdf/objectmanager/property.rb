...
module RDF
  ...
  module AssociatedProperty
    ...
    def []=(md5_or_value,new_value)
      ...
      @subject.send("before_delete_property_#{self.property.to_s}".to_sym) #if @subject.respond_to?"before_delete_property_#{self.property.to_s}".to_sym
      ActiveRDF::FederationManager.delete(@subject, self.property, value)
      @subject.send("after_delete_property_#{self.property.to_s}".to_sym) #if @subject.respond_to?"after_delete_property_#{self.property.to_s}".to_sym
      add(new_value)
    end
    ...
    def add(*args)
      ...
      @subject.send("before_add_property_#{self.property.to_s}".to_sym) #if @subject.respond_to?"before_add_property_#{self.property.to_s}".to_sym
      ...
      @subject.send("after_add_property_#{self.property.to_s}".to_sym) #if @subject.respond_to?"after_add_property_#{self.property.to_s}".to_sym
      
      self
    end

    alias :<< :add #Mauricio: mentém compatibilidade com array
    ...
    def each(&block)  # :yields: value      
      ...
      unless (@subject.datasets.nil? || @subject.datasets.empty?)
        q.datasets(*(@subject.datasets))      
      end
    ...
    #Mauricio: adicionei este método para mostrar o valor da propriedade
