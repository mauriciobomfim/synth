  ...
  class Query2SPARQL
    ...
    def Query2SPARQL.translate(query, engine=nil)
      ...
      if query.select?
        ...
        str << "WHERE { #{where_clauses(query)} #{order_variables(query)} . #{filter_clauses(query)} } "
    ...
    def Query2SPARQL.order_clauses(query)
      query.sort_clauses.collect{|c,order| 
      if c.is_a?RDF::Property
        localname = ActiveRDF::Namespace.localname(c)
        order == :desc ? "DESC(?#{localname})" : "?#{localname}"
      else
        "#{c} ?#{c.gsub(/:/, '_')}"
        order == :desc ? "DESC(?#{c.gsub(/:/, '_')})" : "?#{c.gsub(/:/, '_')}"
      end
      }.join(" ")
    end

    def Query2SPARQL.order_variables(query)
      order_variables = query.sort_clauses.collect{|c,order| 
        if c.is_a?RDF::Property
          "<#{c.uri}> ?#{ActiveRDF::Namespace.localname(c)}"
        else
          "#{c} ?#{c.gsub(/:/, '_')}"
        end
      }.join("; ")
      order_variables = order_variables.blank? ? '' : "; #{order_variables}"
      order_variables
    end
